---
title: Development Docs
icon: ph:code-bold
dir:
    order: 1
---

# Development Docs

Welcome to the MaaKEDR development documentation.

## Navigation

- [Setup & Development Quickstart](./setup.md) — Environment setup, first pipeline, contributing
- [Pipeline Guide](./pipeline.md) — Node structure, recognition types, design patterns
- [Custom Recognition & Action](./custom.md) — Custom module development
- [Vibe Coding](./vibe-coding.md) — AI-assisted development approach
- [Troubleshooting](./fix.md) — Common issues and solutions
- [Formatting](./formatting.md) — Code style and tool configuration
- [Project Structure](./structure.md) — Directory structure and module organization
- [Writing Docs](./doc.md) — Docs site conventions

Task and resource conventions: [Protocol](../protocol/).
