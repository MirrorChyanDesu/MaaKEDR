  ...
  parser: {
    customAction: function (name, param, utils) {
      ...
      const pushTaskRef = (node: PropSelectorResult['node']) => {
        result.push({ node, type: 'taskRef', missingPolicy: 'error' })
      }

      if (name === 'DisableNode') {
        for (const [key, obj] of utils.parseObject(param)) {
          if (key === 'node_name') {
            if (utils.isString(obj)) pushTaskRef(obj)
          }
        }
        ...
        for (const [, overrideNode, propNode] of utils.parseObject(param)) {
          ...
          if (overrideNode.type === 'object') {
            ...
            for (const ref of info.refs) {
              ...
              if (!target || !utils.isString(ref.location)) continue
              if (target === ref.location.value) pushTaskRef(ref.location)
              else pushTaskRef({ ...ref.location, value: target })
            }
          }
        }
        ...
        for (const [key, obj] of utils.parseObject(param)) {
          if (key === 'nodes') {
            for (const task of utils.parseArray(obj)) {
              if (utils.isString(task)) pushTaskRef(task)
            }
          }
        }
